...
- **WHEN** a signed-in user starts a new record from the match detail page
- **THEN** the frontend SHALL open the editing state with create-mode defaults for the current match
...
- **THEN** the frontend SHALL open the same editing state prefilled from the current-user check-in payload
